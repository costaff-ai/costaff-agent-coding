...
This is the coding-side mirror of the BA full treatment: own MCP via a
single SSE McpToolset, the 4 shared tools via this shim — so the agent
holds exactly ONE McpToolset session and the streamable-http/SSE
multi-session race (#4454) cannot occur.
"""
import os
...
def send_message_now(
    ...
    [PROGRESS_CONTEXT] block. Prefix body with [Coding], plain text, no
...
def list_data_files(path: str, pattern: str = None) -> str:
    ...
    path: absolute path under /app/data. pattern: optional glob (e.g. '*.csv').
    """
